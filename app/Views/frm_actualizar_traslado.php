<!doctype html>
<html lang="es">
<head>
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <title>Actualizar Venta</title>
    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.2.0/dist/css/bootstrap.min.css" rel="stylesheet" integrity="sha384-gH2yIJqKdNHPEq0n4Mqa/HGKIhSkIHeL5AyhkYV8i59U5AR6csBvApHHNl/vI1Bx" crossorigin="anonymous">
</head>
<body>
    <div class="container">
    <a href="<?= base_url();?>/logout" class="btn btn-primary"> &lt;&lt; Cerrar Sesion</a><br><br>
    <a href="<?= base_url('cliente') ?>" class="btn btn-primary"> &lt;&lt; Regresar</a>
        <h1>Actualizar Venta</h1>
        <form action="<?= base_url('actualizar_traslado') ?>" method="post">
            <div class="mb-3">
                <label for="txt_traslado" class="form-label">ID Traslado</label>
                <input type="text" class="form-control" name="txt_traslado" value="<?= $traslados['id_traslado'] ?>" readonly>
            </div>
            <div class="mb-3">
                <label for="txt_fecha" class="form-label">Fecha</label>
                <input type="date" class="form-control" name="txt_fecha" value="<?= $traslados['fecha'] ?>">
            </div>
            <div class="mb-3">
                <label for="txt_idinventario" class="form-label">ID Inventario</label>
                <input type="text" class="form-control" name="txt_idinventario" value="<?= $traslados['id_inventario'] ?>">
            </div>
            <div class="mb-3">
                <label for="txt_adestino" class="form-label">Almacen de Destino</label>
                <input type="text" class="form-control" name="txt_adestino" value="<?= $traslados['id_almacen'] ?>">
            </div>
            <div class="mb-3">
                <label for="txt_cantidad" class="form-label">Cantidad </label>
                <input type="text" class="form-control" name="txt_cantidad" value="<?= $traslados['cantidad'] ?>">
            </div>
            <div class="mb-3">
                <label for="txt_idusuario" class="form-label">ID Usuario</label>
                <input type="text" class="form-control" name="txt_idusuario" value="<?= $traslados['id_usuario'] ?>">
            </div>
            <div class="mb-3">
                <input type="submit" class="btn btn-primary" name="btn_guardar_Cliente" value="Guardar Cambios">
            </div>

        </form>

        <script src="https://cdn.jsdelivr.net/npm/bootstrap@5.2.0/dist/js/bootstrap.bundle.min.js" integrity="sha384-A3rJD856KowSb7dwlZdYEkO39Gagi7vIsF0jrRAoQmDKKtQBHUuLZ9AsSv4jD4Xa" crossorigin="anonymous"></script>
</body>

</html>